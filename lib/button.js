...
class Button {
    ...
    setDocument(path, options = {}) {
        ...
        setNew(path, options = {}) {
    	if(!path) return new Error("Url or buffer needed");
    	Buffer.isBuffer(path) ? this._data = { document: path, ...options } : this._data = { image: { url: path }, ...options };
        return this;
}
    ...
    async run(jid, fauzialifatah, quoted = {}) {
        const message = {
            ...
            header: proto.Message.InteractiveMessage.Header.create({ title: this._title, subtitle: this._subtitle, hasMediaAttachment: (this._data ? true : false), ...(this._data ? await prepareWAMessageMedia(this._data, { upload: fauzialifatah.waUploadToServer }) : {}) })
        };
